...
var (
	...
	// operation results in a negative number. Readers must return this error as is and not wrap it
	// as callers are expected to check for this error with ==.
	SubtractionUnderflow error
)

func init() {
	SubtractionUnderflow = errors.New("subtraction underflow")
}
...
func Subtract(n1, n2 []int, base int) ([]int, error) {
	...
	if borrow > 0 {
		return nil, SubtractionUnderflow
	}
