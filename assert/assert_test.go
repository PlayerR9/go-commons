...
func TestAssert(t *testing.T) {
	...
	Assert(NewCondition("foo == \"bar\""), foo == "bar")
}
...
/* func TestAssertF(t *testing.T) {
	defer func() {
		...
		err, ok := r.(*internal.ErrAssertionFailed)
		if !ok {
			t.Errorf("Expected an error of type *ErrAssertionFailed but got %T", r)
		}
	...
	AssertF(Condition, foo == bar, "%s is not %s", foo, bar)
} */
...
func TestAssertErr(t *testing.T) {
	...
	AssertErr(err, NewFunction("my_function", "%s, %s", foo, "bar"))
}
